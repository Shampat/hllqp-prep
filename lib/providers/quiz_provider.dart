...
class QuizProvider extends ChangeNotifier {
  ...
  bool get isLastQuestion => _questions.isEmpty || _currentIndex == _questions.length - 1;
  ...
  Future<void> saveProgress() async {
    if (_currentModule == null || _currentModule!.id == 'mock') return;
  ...
  List<Question>? _restoreSavedOrder(List<Question> questions, String? savedOrder) {
  ...
  Future<void> loadModule(
    ...
    try {
      final data = await rootBundle.loadString(module.assetFile);
      final List<dynamic> jsonList = json.decode(data);
      var loadedQuestions = jsonList.map((e) => Question.fromJson(e)).toList();

      // A free preview is a stable subset, not a fresh random sample. This
      // prevents repeated launches from gradually exposing the full paid bank.
      if (maxQuestions != null && loadedQuestions.length > maxQuestions) {
        loadedQuestions.sort((a, b) => a.id.compareTo(b.id));
        loadedQuestions = loadedQuestions.take(maxQuestions).toList();
      }
      ...
      if (restoredOrder != null) {
        ...
        _questions = List<Question>.from(loadedQuestions)..shuffle();
      }
      ...
      if (canResumeSavedAnswers) {
        ...
        if (savedIdx != null && savedIdx >= 0 && savedIdx < _questions.length && savedAns != null) {
          try {
      ...
      if (!canResumeSavedAnswers) {
        ...
        await prefs.setString('${module.id}_answers', json.encode(_userAnswers));
  ...
  void selectAnswer(int index) {
    final prev = _userAnswers[_currentIndex];
    if (prev != -1 && prev == currentQuestion.correctAnswer) _score--;
  ...
  Future<void> reset() async {
    ...
    _showExplanation = false;
    _userAnswers = List.filled(_questions.length, -1);
    if (_currentModule != null && _currentModule!.id != 'mock') {
      final prefs = await SharedPreferences.getInstance();
